...
# Real values not supplied yet — placeholders, easy to swap once known.
CLINIC_NAME = "RANCO DENTAL"
CLINIC_TAGLINE = "Dental Clinic &amp; Oral Care Centre"
CLINIC_ADDRESS = "[Clinic Address Line 1], [City, State &ndash; PIN]"
CLINIC_PHONE = "[XXXXXXXXXX]"
CLINIC_EMAIL = "[clinic@email.com]"
DOCTOR_QUALIFICATION = "BDS, MDS"
DOCTOR_REG_PREFIX = "Reg. No: [XXXXXXX]"
...
.clinic-header {{ border-bottom: 2px solid {ACCENT}; padding-bottom: 10px; margin-bottom: 16px; }}
.clinic-header table {{ width: 100%; }}
.clinic-name {{ font-size: 22pt; font-weight: bold; color: {ACCENT}; margin: 0; }}
.clinic-sub {{ font-size: 9pt; color: {INK_SOFT}; margin: 2px 0 0; }}
.clinic-contact {{ font-size: 8.5pt; color: {INK_SOFT}; text-align: right; }}
.doc-title {{ font-size: 14pt; font-weight: bold; color: {ACCENT_DEEP}; margin: 0 0 4px; }}
...
.footer {{ font-size: 8pt; color: {INK_SOFT}; border-top: 1px solid {RULE}; padding-top: 6px; margin-top: 24px; font-style: italic; text-align: center; }}
"""
...
def _clinic_header_html(doc_title: str) -> str:
    return f"""
    <div class="clinic-header">
      <table>
        <tr>
          <td>
            <p class="clinic-name">{CLINIC_NAME}</p>
            <p class="clinic-sub">{CLINIC_TAGLINE}</p>
          </td>
      ...
      <p class="doc-title">{_esc(doc_title)}</p>
    </div>
    """
...
def _prescription_entry_html(entry, doctor_name: str, doctor_specialty: str | None) -> str:
    rx_lines = "".join(
    ...
    <div class="entry">
      <div class="entry-head">
        <b>Dr. {_esc(_display_doctor_name(doctor_name))}</b>{specialty_str} &middot; {DOCTOR_QUALIFICATION} &middot; {DOCTOR_REG_PREFIX}
        &mdash; <b>{entry.created_at.strftime('%d %b %Y')}</b>
      </div>
      {diagnosis_html}
...
def render_prescription_pdf(patient, entries: list, staff_by_id: dict) -> bytes:
    """entries: PrescriptionEntry rows for this patient, any order — sorted newest first here."""
    ...
    if not body:
        body = '<p style="color:#8894a3;">No prescriptions recorded yet.</p>'

    html = f"""
    <html><head><style>{BASE_CSS}</style></head>
    <body>
      {_clinic_header_html("Prescription History")}
      {_patient_info_html(patient)}
      {body}
      <div class="footer">This document is valid only with the doctor's signature and clinic stamp. Please follow dosage instructions carefully.</div>
    </body></html>
    """
    return _to_pdf(html)
...
def render_history_pdf(
    ...
    <html><head><style>{BASE_CSS}</style></head>
    <body>
      {_clinic_header_html("Complete Patient History")}
      {_patient_info_html(patient)}
